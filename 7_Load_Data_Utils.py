...
class LoadData(Dataset):
    def __init__(self, txt_path, train_flag=True):
        self.imgs_info = self.get_images(txt_path)  # imgs_info: [[图片完整路径, 标签字符串], ...]
        self.train_flag = train_flag                # True: 训练模式（增强） or False: 验证模式（仅基本变换）
        self.img_size = 224
        ...
        self.train_tf = transforms.Compose([
                ...
                transforms.RandomResizedCrop(self.img_size, scale=(0.8, 1.0)),  # 随机裁剪缩放
    ...
    def get_images(self, txt_path):
        ...
        return imgs_info # 返回图片信息：[图片完整路径, 标签字符串]
...
if __name__ == "__main__":
    train_dataset = LoadData("train.txt", True)     # Tensor, int
